...
export function initializePrompts() {
  ...
  registerPromptTemplate(PromptNames.STORY_OUTLINE, {
    ...
    userPrompt: `根据以下故事设定，为一部 {{chapters_count}} 章的网文创建详细的故事大纲. The outline should be comprehensive, covering main plot points, character arcs, subplots, and key events for each part of the story (beginning, middle, and end).

故事设定： "{{story_premise}}"

**🚫 关键：禁用词汇 - 绝对不要在任何地方使用:**
绝不在大纲的任何地方使用这些词汇 (including character names, place names, or descriptions):
- ""obsidian"（黑曜石）或任何衍生词 (obsidian-like, Obsidian as name)
- ""thorn"、"thorne"（刺）或任何衍生词 (thorns, thorny, Thorne as name, Thornfield, etc.)
- ""crystalline"（水晶般）、"gossamer"（薄如蝉翼）、"eldritch"（诡异）、"ephemeral"（短暂）、"ethereal"（空灵）、"luminescent"（发光）

使用替代词： ""黑石"、"尖刺"、"锐点"、"清澈"、"薄"、"奇怪"、"短暂"、"微弱"、"发光"
这适用于大纲的所有部分，包括角色名和地点。

请用以下明确标记的部分结构化你的回答：
1.  **故事梗概：** 一句话概括核心冲突.
2.  **主要角色：** 列出主要角色，每个角色2-3句简介 of their personality, motivation, and core conflict/arc.
3.  **故事架构（三幕式结构）：**
    *   **第一幕（开篇）：** Introduction to the world and characters, the inciting incident, and the protagonist's initial goal. (Covers roughly the first 25% of chapters).
    *   **第二幕（发展）：** Rising action, new challenges, character development, introduction of allies and enemies, the midpoint (a major turning point), and escalating stakes. (Covers roughly the next 50% of chapters).
    *   **第三幕（高潮）：** The climax, falling action, and the final resolution of the main plot and character arcs. (Covers roughly the final 25% of chapters).
4.  **世界观设定：** 设定、力量体系（如有）、文化等的关键细节
5.  **重复主题/母题：** 列出3-5个将贯穿叙事的反复符号、想法或主题 that will be woven throughout the narrative.`
  });
